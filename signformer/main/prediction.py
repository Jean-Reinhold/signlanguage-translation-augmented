...
def validate_on_data(
    ...
    return_attention_type: str = None,
            ...
            for p in paths:
                ...
                for s in iter_dataset_file(p):
                    ...
                    if len(buffer) >= stream_chunk_size:
                        ...
                        ds_chunk = ttdata.Dataset(examples, named_fields)
                        it = make_data_iter(
                            dataset=ds_chunk,
    ...
    with torch.no_grad():
        ...
        total_num_seqs = 0
        # Accumulate references when streaming
        gls_ref_all = []
        txt_ref_all = []
        for valid_batch in _iter_valid_batches():
            batch = Batch(
                is_train=False,
            ...
            (
                ...
                # return_attention_type=return_attention_type,
            )
            ...
            # Collect references in original order when streaming
            if do_recognition and hasattr(batch, "gls") and isinstance(data, dict):
                # decode gls references from indices
                decoded_gls_ref = model.gls_vocab.arrays_to_sentences(arrays=batch.gls.cpu().numpy())
                gls_ref_all.extend([" ".join(decoded_gls_ref[sri]) for sri in sort_reverse_index])
            if do_translation and hasattr(batch, "txt") and isinstance(data, dict):
                decoded_txt_ref = model.txt_vocab.arrays_to_sentences(arrays=batch.txt.cpu().numpy())
                join_char = " " if level in ["word", "bpe"] else ""
                refs = [join_char.join(decoded_txt_ref[sri]) for sri in sort_reverse_index]
                if level == "bpe":
                    refs = [bpe_postprocess(v) for v in refs]
                txt_ref_all.extend(refs)
        ...
        if do_recognition:
            ...
            if isinstance(data, dict):
                gls_ref = [gls_cln_fn(v) for v in gls_ref_all]
            else:
    ...
    import os, datetime
    if dataset_name is not None and return_attention_type is not None:
        fname = f'output_data_{dataset_name}_{return_attention_type}_{datetime.date.today()}.pkl'
        if not os.path.exists(fname):
            with open(fname, 'wb') as f:
                pickle.dump({'all_txt_outputs': all_txt_outputs,
                            'all_attention_scores': all_attention_scores}, f)
...
def test(
    ...
    if output_path is not None:
        # Helper to get sequence ids for both standard and streaming datasets
        def _collect_sequence_ids(ds) -> List[str]:
        ...
        dev_seq_ids = _collect_sequence_ids(dev_data)
        test_seq_ids = _collect_sequence_ids(test_data)
